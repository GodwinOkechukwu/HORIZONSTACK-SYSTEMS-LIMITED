...
This guide explains how to build and run the horizonstack Digital Innovations application using Docker.
...
```bash
docker build -t horizonstack:latest .
```

Or with a specific version tag:

```bash
docker build -t horizonstack:1.0.0 .
```
...
```bash
docker run -d \
  --name horizonstack_app \
  -p 4019:4019 \
  --env-file .env.local \
  horizonstack:latest
```

Replace `horizonstack:latest` with your image name/tag.
...
| `TABLE_PREFIX`         | Database table prefix                | No       | `horizonstack_`            |
...
docker exec -it horizonstack_postgres psql -U postgres -d store_db
...
2. Or with `docker run`:
   ```bash
   docker run -p 8000:4019 horizonstack:latest
   ```
...
```bash
docker exec horizonstack_postgres psql -U postgres -d store_db -c "\dt"
```
...
```bash
docker rmi horizonstack:latest
```
