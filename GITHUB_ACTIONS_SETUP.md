...
This guide explains how to set up the GitHub Actions workflows for automated deployment of the horizonstack Digital Innovations application.
...
```bash
# Staging
docker logs -f horizonstack-staging

# Production
docker logs -f horizonstack-production
```
...
- Docker image tag: `slemfowel1/horizonstack-digital-innovations:staging`
- Container name: `horizonstack-staging`
- Port: `4019`
...
- Container name: `horizonstack-production`
- Port: `4019`
...
```bash
docker logs horizonstack-staging
```
...
docker exec horizonstack-production npm run migrate
```
...
- name: Run Database Migrations
  run: docker exec horizonstack-production npm run db:migrate
```
...
```bash
# On server
docker stop horizonstack-production
docker rm horizonstack-production
docker pull slemfowel1/horizonstack-digital-innovations:previous_tag
docker run -d -p 4019:4019 --name horizonstack-production \
  --env-file /home/apps/.env.production \
  slemfowel1/horizonstack-digital-innovations:previous_tag
```
